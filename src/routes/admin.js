...
export async function adminRoutes(app) {
  ...
  // Integrated generator: stays inside /admin UI.
  app.post('/api/admin/accesses/manual', async (request, reply) => {
    const admin = await requireAdmin(app, request, reply, { csrf: true });
    if (!admin) return;

    const email = normalizeEmail(request.body?.email);
    if (!email || !email.includes('@')) {
      return reply.code(400).send({ ok: false, message: 'Email no válido.' });
    }
    ...
    const result = await withTransaction(app.db, async (db) => {
      ...
      await writeAudit(db, app, request, admin, inserted.rows[0].id, 'manual_access_created', { email });
    ...
    return reply.send({
      ...
      email,
      code: displayCode,
      accessUrl: `${app.config.appUrl}/access`
    });
  });
  ...
  app.get('/api/admin/access/:id', async (request, reply) => {
    ...
    if (!UUID_PATTERN.test(accessId)) {
      return reply.code(400).send({ ok: false, message: 'Identificador no válido.' });
    }
    ...
    if (!access.rows[0]) {
      return reply.code(404).send({ ok: false, message: 'Acceso no encontrado.' });
    }
  ...
  app.post('/api/admin/access/:id/action', async (request, reply) => {
    ...
    if (!allowed.has(action)) {
      return reply.code(400).send({ ok: false, message: 'Acción no válida.' });
    }
    ...
    const result = await withTransaction(app.db, async (db) => {
      ...
      if (action === 'reactivate_access') {
        await db.query(
          `UPDATE accesses SET status = CASE WHEN activated_at IS NULL THEN 'issued' ELSE 'active' END WHERE id = $1`,
          [accessId]
        );
      }
      ...
      if (action === 'mark_suspicious') {
        await db.query(
          `UPDATE accesses SET suspicious = true, risk_score = GREATEST(risk_score, 50) WHERE id = $1`,
          [accessId]
        );
      }
      ...
      if (action === 'clear_suspicious') {
        await db.query(
          `UPDATE accesses SET suspicious = false, risk_score = 0, risk_notes = NULL WHERE id = $1`,
          [accessId]
        );
      }
    ...
    if (result.invalid) {
      return reply.code(400).send({ ok: false, message: 'La acción no se puede aplicar.' });
    }
    return reply.send({ ok: true });
  });
}
