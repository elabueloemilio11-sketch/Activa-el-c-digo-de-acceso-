...
async function createNewDeviceAndSession(db, app, request, accessId) {
  ...
  await recordSecurityEvent(db, {
    ...
    country,
    metadata: { label: deviceLabel(ua), verification: 'access_code_only' }
  });

  return { accessId, deviceId, deviceToken, ...session };
}
...
export async function accessRoutes(app) {
  app.post('/api/access/validate', async (request, reply) => {
    ...
    if (existingDeviceToken) {
      ...
      if (existing.rows[0]) {
        const session = await withTransaction(app.db, async (db) => {
          const created = await createSessionRecord(db, app.config, request, {
            accessId: access.id,
    ...
    const outcome = await withTransaction(app.db, async (db) => {
      await db.query('SELECT id FROM accesses WHERE id = $1 FOR UPDATE', [access.id]);
      ...
      if (Number(count.rows[0].count) >= Number(access.max_devices)) {
        return { deviceLimit: true };
      }

      const session = await createNewDeviceAndSession(db, app, request, access.id);
      return { session };
    });
  ...
  // Old email / OTP endpoints are intentionally disabled.
  for (const path of [
    '/api/access/activate',
