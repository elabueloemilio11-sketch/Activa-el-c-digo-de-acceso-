(() => {
  ...
  const K_MIGRATION = 'ae_music_player_migration_v9';

  // v9: reset one time to 10%. iOS volume is controlled through Web Audio GainNode.
  ...
  function ensureAudioGraph() {
    ...
    try {
      ...
      // iOS ignores HTMLMediaElement.volume. Keep the element at full scale and
      // apply the requested percentage in the GainNode instead.
      audio.volume = 1;
      audio.muted = false;
      return true;
    } catch (_) {
  ...
  function applyVolume() {
    ...
    // Fallback for browsers where Web Audio is unavailable.
    audio.volume = gain;
    audio.muted = muted;
  }
