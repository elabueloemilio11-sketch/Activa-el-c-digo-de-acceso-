...
async function loadDetail(id) {
  ...
  actions.append(
    ...
    actionButton(
      access.status === 'disabled' ? 'REACTIVAR ACCESO' : 'DESACTIVAR ACCESO',
      access.status === 'disabled' ? 'reactivate_access' : 'disable_access',
      access.status !== 'disabled'
    ),
    actionButton(
      access.suspicious ? 'LIMPIAR ALERTA SUSPICIOUS' : 'MARCAR COMO SUSPICIOUS',
      access.suspicious ? 'clear_suspicious' : 'mark_suspicious'
    ),
...
async function runAction(action, extra, button) {
  ...
  try {
    await api(`/api/admin/access/${selectedAccessId}/action`, {
      method: 'POST',
      body: { action, ...extra }
    });
...
$('#code-generator-form').addEventListener('submit', async (event) => {
  event.preventDefault();
  const button = $('#generator-submit');
  button.disabled = true;
  adminMessage('');
  try {
    const body = await api('/api/admin/accesses/manual', {
      method: 'POST',
      body: { email: $('#generator-email').value }
    });
    lastGenerated = body;
    $('#generated-email').textContent = body.email;
    $('#generated-code').textContent = body.code;
    $('#generated-result').hidden = false;
    adminMessage('Código creado correctamente.', true);
    await loadAccesses($('#admin-search').value);
  } catch (error) {
    adminMessage(error.message);
  } finally {
    button.disabled = false;
...
$('#copy-generated-code').addEventListener('click', async () => {
  if (!lastGenerated) return;
  await navigator.clipboard.writeText(lastGenerated.code);
  adminMessage('Código copiado.', true);
});

$('#copy-access-data').addEventListener('click', async () => {
  if (!lastGenerated) return;
  await navigator.clipboard.writeText(
    `Abuelo Emilio Academy\nEmail: ${lastGenerated.email}\nCódigo: ${lastGenerated.code}\nAcceso: ${lastGenerated.accessUrl}`
  );
  adminMessage('Código y enlace copiados.', true);
});
